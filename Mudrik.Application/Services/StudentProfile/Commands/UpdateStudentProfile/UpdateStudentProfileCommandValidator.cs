...
public class UpdateStudentProfileCommandValidator : AbstractValidator<UpdateStudentProfileCommand>
{
    ...
    private static readonly string[] ValidFontPreferences = { "OpenDyslexic", "Arial", "Roboto" };
    private static readonly string[] ValidColorOverlays = { "none", "cream", "blue", "green", "pink", "yellow" };

    public UpdateStudentProfileCommandValidator()
    {
        RuleFor(x => x.id)
            .GreaterThan(0)
            .WithMessage("معرف الطالب غير صالح.");
        ...
        RuleFor(x => x.FontPreference)
            .NotEmpty()
            .WithMessage("تفضيل الخط مطلوب.")
            .Must(v => ValidFontPreferences.Contains(v))
            .WithMessage($"الخط يجب أن يكون أحد: {string.Join(", ", ValidFontPreferences)}.");

        RuleFor(x => x.ColorOverlay)
            .NotEmpty()
            .WithMessage("تفضيل لون الخلفية مطلوب.")
            .Must(v => ValidColorOverlays.Contains(v?.ToLower()))
            .WithMessage($"لون الخلفية يجب أن يكون أحد: {string.Join(", ", ValidColorOverlays)}.");
